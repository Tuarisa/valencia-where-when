...
// T207 — year inference is anchored to the POST date, never to "now" at normalize
// time, via the shared `anchoredPostDate` (post-date anchor + 60-day roll-forward
// guard; born here as `anchoredExcursionDate`, extracted to shared.ts in T209 so
// every telegram sibling shares the exact same year semantics).
